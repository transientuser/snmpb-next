...
Downloaded content is stored below Qt's per-user `AppLocalDataLocation`:
...
The downloaded directory is appended to libsmi's effective path at runtime and
is not written into `mibpaths`. Existing configured paths remain untouched
...
bundled paths remain dynamically computed by Preferences. Bundled files are
never written or replaced.
