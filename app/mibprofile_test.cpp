...
int main(int argc,char**argv){QCoreApplication app(argc,argv);bool ok=true;QTemporaryDir temp;
 ...
 MibProfileService legacy{MibProfileRepository(legacyPath)};ok&=check(legacy.migrateLegacyFolderProfiles(&error)&&legacy.find("legacy-folder")&&legacy.find("legacy-folder")->type==MibProfileType::Custom,"legacy folder Profile migrates to editable snapshot with stable ID");const auto migrated=legacy.find("legacy-folder")->members;writeFile(QDir(folder).filePath("post.mib"),mib("POST-MIB"));legacy.migrateLegacyFolderProfiles(&error);ok&=check(legacy.find("legacy-folder")->members==migrated,"migrated Profile never regenerates");
